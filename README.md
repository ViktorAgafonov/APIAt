...
Команды оператора (email):
   ...
   расписание: ...  -> автозапуск навыков/цепочек по daily/weekly
   анализ логов     -> journalctl + LLM-анализ (на сервере, не в интернете)
```
...
- `skills/` — система навыков: генерация, ревью, Docker sandbox, подтверждение, scheduler.
...
| `расписание: каждый день в 18:00 запускай навык X` | Создать расписание автозапуска (LLM парсит текст) |
| `расписание: покажи` | Список всех расписаний |
| `расписание: останови <имя>` | Приостановить расписание |
| `расписание: запусти <имя>` | Возобновить расписание |
| `расписание: удали <имя>` | Удалить расписание |
| `анализ логов` / `логи приложения` | journalctl + LLM-анализ ошибок на сервере |
| `статус сервиса` | systemctl status apiat |
| `место на диске` | df -h |
| `процессы` / `что грузит память` | Топ процессов по памяти |
...
### Расписания (автозапуск по времени)

```
расписание: каждый день в 18:00 запускай навыки fetch_ru_news и fetch_world_news
  → LLM парсит текст → Schedule{name, schedule, tasks, owner}
  → Сохраняется в data/schedules/<name>.schedule.json
  → Daemon-цикл проверяет каждую минуту
  → При срабатывании — запуск задач, отчёт оператору-создателю

запланируй: каждую субботу в 09:00 запускай цепочку weekly_digest
  → LLM парсит → Schedule с type=chain

расписание: покажи          → список всех расписаний
расписание: останови <имя>  → приостановить
расписание: запусти <имя>   → возобновить
расписание: удали <имя>     → удалить
```
...
**Уведомления** — только оператору-создателю расписания. Рассылка по whitelist — только через явный навык.

### Серверные задачи

```
анализ логов приложения
  → ServerTool: journalctl -u apiat -n 100
  → LLM анализирует: ошибки, предупреждения, аномалии
  → Отчёт оператору с рекомендациями

статус сервиса     → systemctl status apiat
место на диске     → df -h /
процессы           → ps aux --sort=-%mem (топ-15)
```

Серверные задачи выполняются **на самом сервере**, а не в интернете. LLM-парсер отличает
`анализ логов приложения` (→ server) от `статья про анализ логов` (→ search).
...
**Порядок обработки запроса в письме**
Если навыков нет совсем — LLM-шаг пропускается и запрос идёт в обычный LLM-парсинг (search/youtube/...). Лишний вызов LLM только когда есть закреплённые навыки и точное совпадение не найдено.

"статус сервера"
    ↓ regex команд — не сработал
    ↓ _match_skill:
        1. точное: "статус_сервера" ∈ "server_status"? → нет
        2. LLM: "Доступные навыки: server_status. Запрос: статус сервера..."
               → LLM отвечает "server_status"
    ↓ _handle_run_skill("server_status") → Docker sandbox → ответ
...
- [x] Цепочки навыков (chain planner + save/run)
...
- [x] Scheduler: автозапуск навыков/цепочек по daily/weekly расписанию
- [x] ServerTool: анализ логов journalctl, статус сервиса, диски, процессы
- [x] Поиск: только реальные RSS-данные + LLM-пересказ (без фантазий, без ссылок)
