...
SYSTEM_PROMPT = (
    ...
    "9. Для type=news ОБЯЗАТЕЛЬНО заполни topic ключевыми словами для поиска (например 'новости сегодня', 'технологии', 'спорт'). Для type=search ОБЯЗАТЕЛЬНО заполни query поисковым запросом. НЕ оставляй topic/query пустым — извлеки суть запроса оператора.\n\n"
