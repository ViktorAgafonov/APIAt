...
# Команда самообучения: "самообучись: <описание>" / "создай навык: <описание>"
_LEARN_CMD_RE = re.compile(
    r"(самообучись|learn|создай\s+навык|создай\s+цепочку|make\s+skill)\s*[:\-]?\s*(.+)",
    re.IGNORECASE,
)
...
class Agent:
    ...
    def _handle_help(self, mail: IncomingMail) -> None:
        ...
        lines = [
            ...
            "  поиск / новости   — реальные новости через Google News RSS + LLM-пересказ",
            "                      (без ссылок — оператор в зоне блокировок)",
            ...
            "",
            "## Серверные задачи (на самом сервере, не в интернете)",
            "  анализ логов / логи приложения  — journalctl + LLM-анализ ошибок",
            ...
            "  создай навык: <описание>         — то же самое",
            ...
            "  создай цепочку: <задача>        — то же самое",
            ...
            "  расписание: каждый день в 18:00 запускай навык fetch_news  — создать",
            "  запланируй: каждую субботу в 09:00 запускай цепочку digest  — создать",
            ...
            "  Уведомления о выполнении — только оператору-создателю",
            "",
            "## Анализ переписки",
